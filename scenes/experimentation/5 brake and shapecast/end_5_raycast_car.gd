extends RigidBody3D
class_name End5RaycastCar

@export var disabled := false
@export var wheels: Array[End5RaycastWheel]
...
func _basic_steering_rotation(wheel: End5RaycastWheel, delta: float) -> void:
...
func _physics_process(delta: float) -> void:
	if disabled: return
	if show_debug: DebugDraw.draw_arrow_ray(global_position, linear_velocity, 2.5, 0.15, Color.GREEN)
