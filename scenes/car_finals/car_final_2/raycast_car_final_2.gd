...
const RaycastWheelFinal = preload("uid://cjcsfwc2cvokt")


@export var wheels: Array[RaycastWheelFinal]
...
func _do_single_wheel_acceleration(ray: RaycastWheelFinal) -> void:
	...
	if ray.is_colliding():
		...
		#contact = ray.get_collision_point()
...
func _do_single_wheel_suspension(ray: RaycastWheelFinal) -> void:
	if ray.is_colliding():
		ray.target_position.y = -(ray.rest_dist + ray.wheel_radius + ray.over_extend)
		...
		spring_len = max(spring_len, 0)
		var offset := ray.rest_dist - spring_len

		ray.wheel.position.y = move_toward(ray.wheel.position.y, -spring_len, 5 * get_physics_process_delta_time())
