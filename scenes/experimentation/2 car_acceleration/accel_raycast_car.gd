extends RigidBody3D
const AccelWheel = preload("uid://my0ogf0dfmms")

@export var wheels: Array[AccelWheel]
...
func _do_single_wheel_acceleration(suspension_ray: AccelWheel) -> void:
...
func _do_single_wheel_acceleration_basic(suspension_ray: AccelWheel) -> void:
	var foward_dir := -suspension_ray.global_basis.z
	var vel := foward_dir.dot(linear_velocity)
	var wheel:Node3D = suspension_ray.get_node("Wheel")
	wheel.rotate_x(-2 * vel * get_process_delta_time())
...
func _do_single_wheel_suspension(suspension_ray: AccelWheel) -> void:
